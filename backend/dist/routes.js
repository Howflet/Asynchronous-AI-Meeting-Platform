...
app.get("/api/meetings/:id/status", (req, res) => {
    ...
    const personas = db.prepare("SELECT * FROM personas WHERE meetingId = ?").all(meeting.id);
...
app.post("/api/meetings/:id/pause", requireHost, (req, res) => {
    const meeting = getMeeting(req.params.id);
    if (meeting.status === "paused")
        return res.json({ status: meeting.status });
...
app.post("/api/meetings/:id/inject", (req, res) => {
    ...
    const participant = db.prepare("SELECT id FROM participants WHERE meetingId = ? AND email = ?").get(req.params.id, parse.data.author);
    let speaker;
    if (participant) {
...
app.get("/api/meetings", requireHost, (req, res) => {
    const meetings = db.prepare("SELECT id, subject, details, status, createdAt FROM meetings ORDER BY createdAt DESC").all();
...
app.get("/api/meetings/:id/report", (req, res) => {
    const row = db.prepare("SELECT * FROM reports WHERE meetingId = ?").get(req.params.id);
    if (!row)
