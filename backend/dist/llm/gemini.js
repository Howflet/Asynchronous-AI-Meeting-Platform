...
import { estimateInputTokens, estimateOutputTokens, extractTokenUsage, logTokenUsage, calculateTotalEstimate } from "./tokenEstimator.js";
...
function getClient(useModerator = false) {
...
export async function generatePersonaFromInput(input, meetingSubject, participantName) {
...
Return ONLY this JSON (no markdown, keep it concise):
{
  ...
  "config": {
    ...
    return await participantRateLimiter.scheduleRequest(async () => {
        return await withRetry(async () => {
            ...
            if (actualUsage) {
                ...
                logTokenUsage('generatePersonaFromInput', { input: estimatedInput, output: estimatedOutput, total: totalEstimated }, actualUsage);
            }
            ...
            try {
                ...
                return { name: parsed.name, config: parsed.config };
            }
            catch (parseError) {
        ...
        }, 'generatePersonaFromInput', GEMINI_RETRY_CONFIG);
    }, totalEstimated, 1 // High priority for persona generation
    );
}
export async function moderatorDecideNext(moderatorConfig, whiteboard, history, participantOptions, pendingHumanInjections, meetingSubject, meetingDetails) {
    ...
    const othersWhoSpoke = hasSpoken.filter(email => !lastSpeakerEmail.includes(email) && !email.includes(lastSpeakerEmail.split(' ')[0]));
...
{"nextSpeaker":"email or none","moderatorNotes":"brief","whiteboardUpdate":{"keyFacts":["brief"],"decisions":[],"actionItems":[]}}`;
    ...
    const estimatedOutput = estimateOutputTokens('json');
    const totalEstimated = calculateTotalEstimate(estimatedInput, estimatedOutput);
    return await moderatorRateLimiter.scheduleRequest(async () => {
        return await withRetry(async () => {
            ...
            if (actualUsage) {
                ...
                logTokenUsage('moderatorDecideNext', { input: estimatedInput, output: estimatedOutput, total: totalEstimated }, actualUsage);
            }
            const text = resp.response.text().trim();
        ...
        }, 'moderatorDecideNext', GEMINI_RETRY_CONFIG);
    }, totalEstimated, 2 // Normal priority
    );
}
export async function personaRespond(persona, whiteboard, history, participantInput, webContext) {
...
Max 70 words. Focus on NEW CONTRIBUTIONS not repetition.`;
    ...
    return await participantRateLimiter.scheduleRequest(async () => {
        return await withRetry(async () => {
            ...
            if (actualUsage) {
                participantRateLimiter.reconcileUsage(totalEstimated, actualUsage.totalTokens);
                logTokenUsage('personaRespond', { input: estimatedInput, output: estimatedOutput, total: totalEstimated }, actualUsage);
            }
            const text = resp.response.text().trim();
            ...
            return text;
        }, 'personaRespond', GEMINI_RETRY_CONFIG);
    }, totalEstimated, 2 // Normal priority
    );
}
export async function checkForConclusion(moderatorConfig, whiteboard, history) {
...
{ "conclude": boolean, "reason": "brief explanation under 50 words" }`;
    ...
    return await moderatorRateLimiter.scheduleRequest(async () => {
        return await withRetry(async () => {
            ...
            if (actualUsage) {
                moderatorRateLimiter.reconcileUsage(totalEstimated, actualUsage.totalTokens);
                logTokenUsage('checkForConclusion', { input: estimatedInput, output: estimatedOutput, total: totalEstimated }, actualUsage);
            }
            const text = resp.response.text().trim();
        ...
        }, 'checkForConclusion', GEMINI_RETRY_CONFIG);
    }, totalEstimated, 3 // Lower priority
    );
}
export async function summarizeConversation(whiteboard, history) {
...
{"summary":"100 words","highlights":["point"],"decisions":["decision"],"actionItems":["action"],"visualMap":{"nodes":[],"edges":[]}}`;
    ...
    return await moderatorRateLimiter.scheduleRequest(async () => {
        return await withRetry(async () => {
            ...
            if (actualUsage) {
                moderatorRateLimiter.reconcileUsage(totalEstimated, actualUsage.totalTokens);
                logTokenUsage('summarizeConversation', { input: estimatedInput, output: estimatedOutput, total: totalEstimated }, actualUsage);
            }
            const text = resp.response.text().trim();
        ...
        }, 'summarizeConversation', GEMINI_RETRY_CONFIG);
    }, totalEstimated, 0 // Highest priority for final report
    );
}
