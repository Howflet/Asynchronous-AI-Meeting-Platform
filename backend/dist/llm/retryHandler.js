 ...
 */
function calculateRetryDelay(attemptNumber, config, retryInfo) {
    // Honor provider's retry advice if available
    if (retryInfo?.retryDelayMs) {
        console.log(`[Retry] Using provider-specified delay: ${retryInfo.retryDelayMs}ms`);
    ...
    if (retryInfo?.retryAfter) {
        ...
        console.log(`[Retry] Using Retry-After header delay: ${delayMs}ms`);
    ...
    for (let attempt = 0; attempt <= retryConfig.maxRetries; attempt++) {
        try {
            ...
            if (attempt >= retryConfig.maxRetries) {
                console.error(`[Retry] ${operationName} failed after ${attempt} retries:`, error?.message || error);
                throw error;
            }
            ...
            console.warn(`[Retry] ${operationName} failed (attempt ${attempt + 1}/${retryConfig.maxRetries + 1}). ` +
