...
export class GeminiRateLimiter {
    ...
    lastMinuteReset;
    lastDayReset;
    lastRefillLogTime = 0; // Track when we last logged refill
    lastRequestTime = 0; // Track last request for burst prevention
     ...
     */
    async scheduleRequest(execute, estimatedTokens, priority = 0) {
    ...
    canProcessRequest(estimatedTokens) {
        return (this.requestsAvailable >= 1 &&
     ...
     */
    reserveCapacity(estimatedTokens) {
     ...
     */
    reconcileUsage(estimatedTokens, actualTokens) {
        ...
        if (Math.abs(difference) > estimatedTokens * 0.2) {
            console.warn(`[RateLimiter] Token estimate off by ${difference} ` +
    ...
    async waitForCapacity(estimatedTokens) {
        ...
        console.log(`[RateLimiter] Waiting ${waitTime}ms for capacity. ` +
