...
export async function ensurePersonasForMeeting(meeting) {
    const existing = db.prepare("SELECT * FROM personas WHERE meetingId = ?").all(meeting.id);
    if (existing.length > 0)
        return existing.map(rowToPersona);
...
export function getHistory(meetingId) {
    const rows = db.prepare("SELECT * FROM conversation_turns WHERE meetingId = ? ORDER BY createdAt").all(meetingId);
...
export function appendTurn(meetingId, speaker, message, metadata) {
    const turn = { id: generateId("trn"), meetingId, speaker, message, createdAt: now(), metadata };
    db.prepare("INSERT INTO conversation_turns (id, meetingId, speaker, message, createdAt, metadata) VALUES (?, ?, ?, ?, ?, ?)").run(turn.id, turn.meetingId, turn.speaker, turn.message, turn.createdAt, metadata ? toJson(metadata) : null);
    return turn;
}
/**
 ...
 */
function detectRepetitiveConversation(history) {
...
export async function runOneTurn(meeting, pendingHumanInjections) {
    ...
    try {
        ...
        const currentMeeting = db.prepare("SELECT status FROM meetings WHERE id = ?").get(meeting.id);
        ...
        const personas = db.prepare("SELECT * FROM personas WHERE meetingId = ?").all(meeting.id).map(rowToPersona);
        ...
        if (repetitionCheck.isRepetitive) {
            ...
            const freshHistory = db.prepare("SELECT * FROM conversation_turns WHERE meetingId = ? ORDER BY createdAt DESC LIMIT 5").all(meeting.id);
        ...
        const participantOptions = inputs.map(input => {
            const participant = db.prepare("SELECT email FROM participants WHERE id = ?").get(input.participantId);
        ...
        const decision = await moderatorDecideNext(moderator.config, whiteboard, history, participantOptions, pendingHumanInjections, meeting.subject, meeting.details);
        ...
        console.log(`[ConversationService] Available participants:`, participantOptions.map(p => `${p.email} (spoken: ${p.hasSpoken})`).join(', '));
        ...
        if (speaker.participantId) {
            const input = db.prepare("SELECT content FROM participant_inputs WHERE participantId = ?").get(speaker.participantId);
        ...
        console.log(`[ConversationService] DEBUG: ENABLE_PERSONA_WEB_SEARCH="${process.env.ENABLE_PERSONA_WEB_SEARCH}", researchEnabled=${researchEnabled}`);
        ...
        if (lastUserAsk)
            console.log(`[ConversationService] Found recent user ask: "${lastUserAsk.slice(0, 50)}..."`);
        const web = await maybeResearchForPersona({
            enabled: researchEnabled,
            meetingObjective: `${meeting.subject} ${meeting.details}`.trim(),
            lastUserAsk,
        });
        const msg = await personaRespond({ name: speaker.name, config: speaker.config }, meeting.whiteboard, history, participantInput, web?.compactContext);
        ...
        if (web?.citations?.length) {
            insertCitations(web.citations.map(c => ({
...
export async function attemptConclusion(meeting) {
    ...
    const moderator = db.prepare("SELECT * FROM personas WHERE meetingId = ? AND role = 'moderator'").get(meeting.id);
    if (!moderator)
        return { conclude: false, reason: "Moderator missing" };
    const history = getHistory(meeting.id);
    ...
    return await checkForConclusion(fromJson(moderator.config), meeting.whiteboard, history);
}
export async function generateFinalReport(meeting) {
    ...
    const existingReport = db.prepare("SELECT id FROM reports WHERE meetingId = ?").get(meeting.id);
    if (existingReport) {
        console.log(`[ConversationService] Report already exists for meeting ${meeting.id}, skipping generation`);
        const fullReport = db.prepare("SELECT * FROM reports WHERE id = ?").get(existingReport.id);
        return {
            id: fullReport.id,
    ...
    db.prepare("INSERT INTO reports (id, meetingId, summary, highlights, decisions, actionItems, visualMap, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
        .run(reportId, meeting.id, summary.summary, toJson(summary.highlights), toJson(summary.decisions), toJson(summary.actionItems), toJson(summary.visualMap), now());
