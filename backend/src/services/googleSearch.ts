...
const cache = new Map<string, { expires: number; data: SearchHit[] }>();
...
function rpmGuard() {
  ...
  if (callsThisMinute >= RPM) throw new Error("Search rate limit reached");
  callsThisMinute++;
}

export async function googleSearch(query: string): Promise<SearchHit[]> {
  if (!API_KEY || !CX) throw new Error("Missing GOOGLE_CSE_API_KEY or GOOGLE_CSE_CX");
